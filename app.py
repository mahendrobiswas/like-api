...
def load_accounts(server_name):
    ...
    try:
        ...
        if not os.path.exists(filename):
            print(f"⚠️ {filename} not found, trying account_BD.txt")
            filename = "account_BD.txt"
            if not os.path.exists(filename):
...
def handle_requests():
    ...
    # Valid servers
    valid_servers = ["BD", "BR", "US", "SAC", "NA", "BD","RU"]
    ...
    if not accounts:
        # Try fallback to BD
        accounts = load_accounts("BD")
        if not accounts:
            return jsonify({"error": f"No accounts found for server {server_name}"}), 500
        print(f"⚠️ Using BD accounts as fallback for {server_name}")
...
if __name__ == '__main__':
    ...
    print("📁 Account files:")
    print("   - account_BD.txt (IND server)")
